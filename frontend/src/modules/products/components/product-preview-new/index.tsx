...
export default function ProductPreviewNew({
  product,
  isFeatured,
  region,
}: ProductPreviewProps) {
  ...
  return (
    ...
    >
      ...
      <div className="relative mx-auto col-span-2 w-full h-fit max-w-[300px] bg-gradient-to-br from-slate-50 to-slate-100 overflow-hidden">
        {/* Skeleton Loading */}
        {!imageLoaded && (
        ...
        {/* Border  */}
        <div className="absolute bottom-2 right-2 top-2 left-2 hover:bottom-0 hover:right-0 hover:top-0 hover:left-0 border border-black inset-0 transition-all duration-300" />

        {/* Discount Badge - show only when there is a real discount */}
        {hasDiscount && (
          <div className="absolute top-2 left-0 bg-[#1e1e1e] z-20 text-white px-1 py-1 text-sm font-medium flex items-center justify-center shadow-lg">
      ...
      {/* Content Container */}
      <div className="col-span-3 h-fit flex flex-col items-start flex-1 bg-none">
        {/* Product Type */}
        {product.type && (
        ...
        <div className="h-fit w-full font-nunito">{/* Base Price Badge - bottom right, show only when there is a discount */}
          {hasDiscount ? (
            <div className="h-full w-full flex gap-1">
              ...
              <span className="text-sm font-bold text-[#339994]">
                {formattedDiscountedPrice}
              </span>
            </div>
          ) : (
            <div className="text-sm font-bold text-[#339994]">
              {formattedDiscountedPrice}
            </div>
          )}
        </div>
